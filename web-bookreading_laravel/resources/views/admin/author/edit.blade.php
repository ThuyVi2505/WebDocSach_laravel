...
<div class="container-fluid px-4">
    ...
    <div class="card mt-4">
        <div class="card-header">
            <a href="{{ url('admin/author') }}" class="btn btn-primary btn-md float-end"><i class="fa-solid fa-clipboard-list"></i> Xem Danh sách</a>
        </div>
        <div class="card-body">
            <form action="{{ url('admin/update_author/'.$author->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    <!-- input author -->
                    ...
                    <div class="form-group col-lg-4 col-md-12 col-sm-12">
                        ...
                        <div class="input-group">
                            <label class="input-group-text" for="author_status">Kích hoạt</label>
                            <select class="form-select @error('author_status') is-invalid @enderror" name="author_status" id="author_status">
                                <option value="">Chọn...</option>
                                <option value="1" {{ ($author->author_status == 1) ? 'selected' : '' }}>Có</option>
                                <option value="0" {{ ($author->author_status == 0) ? 'selected' : '' }}>Không</option>
                            </select>
                            @error('author_status')
                            <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                            @enderror
                ...
                <div class="form-group">
                    <button type="submit" name="create_author" class="btn btn-success "><i class="fa-solid fa-check"></i> Cập nhật</button>
                </div>
            </form>
        </div>
    </div>
